...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <form method="GET" action="{{ route('admin.partners.index') }}" class="flex flex-1 max-w-md gap-2">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari partner (nama, email, kontak)..."
            ...
            @if(request('search'))
            <a href="{{ route('admin.partners.index') }}" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-semibold hover:bg-slate-50 transition">Reset</a>
            @endif
        </form>
        <a href="{{ route('admin.partners.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-semibold shadow-md hover:bg-indigo-700 transition">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    ...
                    <tr class="hover:bg-slate-50/80 transition">
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            @if($p->logo_path)
                                <img src="{{ asset('storage/'.$p->logo_path) }}" class="w-12 h-12 rounded-xl object-cover shadow-sm border border-slate-100">
                            @else
                        ...
                        <td class="px-6 py-5">
                            <div class="flex flex-col">
                                ...
                                <div class="flex items-center gap-2 mt-1 flex-wrap">
                                    ...
                                    @if($p->website)
                                    <a href="{{ $p->website }}" target="_blank" rel="noopener" class="text-xs text-indigo-500 hover:text-indigo-700 inline-flex items-center gap-1">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                                        {{ parse_url($p->website, PHP_URL_HOST) }}
                                    </a>
                                    @endif
                                </div>
                            </div>
                        ...
                        <td class="px-6 py-5 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {{ $p->is_active ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' : 'bg-slate-100 text-slate-600 ring-1 ring-slate-200' }}">
                                {{ $p->is_active ? 'Aktif' : 'Tidak Aktif' }}
                            </span>
                        </td>
                        <td class="px-6 py-5 whitespace-nowrap text-right">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('admin.partners.edit', $p) }}" class="p-2 rounded-xl text-indigo-600 hover:bg-indigo-50 transition" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </a>
                                <form action="{{ route('admin.partners.destroy', $p) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus partner ini? Data akan hilang permanen.');">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center gap-3">
                                ...
                                <a href="{{ route('admin.partners.create') }}" class="mt-2 inline-flex items-center gap-1 px-4 py-2 text-sm bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition">+ Tambah Partner Pertama</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
{{-- Optional: extra style to override pagination default (if using Bootstrap or custom) --}}
@push('styles')
<style>
