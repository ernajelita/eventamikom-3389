    ...
    <div class="max-w-md w-full bg-white text-slate-900 rounded-[2rem] p-8 shadow-2xl">
        ...
        <form action="{{ route('admin.login.post') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium @error('email') border-red-500 @enderror" required>
            </div>
            <div>
